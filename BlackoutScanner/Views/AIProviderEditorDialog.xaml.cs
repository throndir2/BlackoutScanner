...
namespace BlackoutScanner.Views
{
    public partial class AIProviderEditorDialog : Window
    {
        ...
        {
            { "Nvidia", new List<string> { "nvidia/nemotron-parse", "baidu/paddleocr", "nvidia/nemoretriever-ocr-v1" } },
            ...
            { "Mistral", new List<string> { "mistral-large-latest", "pixtral-12b-2409", "mistral-small-latest", "mistral-medium-latest", "ministral-8b-latest", "ministral-3b-latest" } },
            { "Cohere", new List<string> { "command-a-vision-07-2025" } },
        ...
        public AIProviderEditorDialog(AIProviderConfiguration? existingConfig = null, int defaultPriority = 1)
        {
            InitializeComponent();
            ...
            // Set default priority for new providers
            if (!_isEditMode)
            {
                _configuration.Priority = _defaultPriority;
            }
        ...
        private void UpdateDisplayNameSuggestion()
        {
            ...
            // Check if display name looks like a previous auto-generated suggestion
            var currentName = displayNameTextBox.Text ?? "";
            var isAutoGenerated = string.IsNullOrWhiteSpace(currentName) ||
                currentName.Contains("NVIDIA") ||
                currentName.Contains("Gemini") ||
                currentName.Contains("OpenAI") ||
                currentName.Contains("Custom") ||
                currentName.Contains("Groq") ||
                currentName.Contains("Mistral") ||
                currentName.Contains("Cohere") ||
                currentName == "AI OCR Provider";
            
            // Always update if it looks auto-generated
            if (isAutoGenerated)
            {
                if (providerTypeComboBox.SelectedItem is ComboBoxItem selectedItem)
                {
                    ...
                    // Extract short model name for cleaner display
                    var shortModelName = modelText;
                    if (modelText.Contains('/'))
                    {
                        shortModelName = modelText.Split('/')[^1];
                    }

                    displayNameTextBox.Text = providerType switch
                    {
                        "Nvidia" => !string.IsNullOrEmpty(modelText) ? $"NVIDIA {shortModelName}" : "NVIDIA OCR",
                        "Gemini" => !string.IsNullOrEmpty(modelText) ? $"Gemini {modelText}" : "Gemini Vision",
                        "Groq" => !string.IsNullOrEmpty(modelText) ? $"Groq {shortModelName}" : "Groq Llama 4",
                        "Mistral" => !string.IsNullOrEmpty(modelText) ? $"Mistral {shortModelName}" : "Mistral Vision",
                        "Cohere" => !string.IsNullOrEmpty(modelText) ? $"Cohere {shortModelName}" : "Cohere Vision",
        ...
        private IAIProvider? GetAIService(AIProviderConfiguration config)
        {
            try
            {
                return config.ProviderType switch
                {
                    ...
                    "Mistral" => new BlackoutScanner.Services.MistralOCRService(),
                    "Cohere" => new BlackoutScanner.Services.CohereOCRService(),
                    // Future providers:
        ...
        private void ConfigureAIService(IAIProvider service, AIProviderConfiguration config)
        {
            ...
            else if (service is BlackoutScanner.Services.MistralOCRService mistralService)
            {
                mistralService.UpdateConfiguration(config.ApiKey, config.Model);
            }
            else if (service is BlackoutScanner.Services.CohereOCRService cohereService)
            {
                cohereService.UpdateConfiguration(config.ApiKey, config.Model);
            }
